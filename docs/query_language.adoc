Zoe - A Query Language
======================
To get data out of the database we need some way to interact with it.
Gravitydb provides such means in the form of a query language named zoe.

This document provides information about how this language is specified
and desinged. To get started with using the language please see TODO.
...
* you have a property which values could be ordered on a scale. So you
  could search all properties in a range defined on that scale
+
[[property_query_fields]]
[source, rust]
----
...
== Prior Art
Es gibt einige Graph-Abfrage Sprachen aber keine große Standardisierung
footnote:[Und dem will ich hier Abhilfe schaffen, durch eine weitere
Sprache :)]. Manche Sprachen haben eine größere Bedeutung erlangt
und/oder es wäre interessant sie näher anzuschauen.
...
https://en.wikipedia.org/wiki/Prolog[Prolog]:: This is a logic
  programming language. While coming from a different background, it
  is amasing how many similarities are between prolog and graph query
  languages. It can definitly serve as a source of inspiration for both
  implementation strategies as well as what could be done when using zoe
