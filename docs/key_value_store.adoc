...
beliebigen Key Value Stores aufbaut. Dadurch kann man verschieden
...
Allerdings möchten wir, dass unsere Datenbank auch verwendbar ist, ohne
das jeder zuerst ein Schema erdenken und anschließend die Datenbank
mit diesem Schema gemeinsam kompilieren muss. Aus diesem Grund
definieren wir ein sehr allgemeines Schema, welches beliebige Daten (wie
bespielsweise Dateien auf dem Computer) aufnehmen kann. Seine sehr laxe
Validierung erlaubt direkt mit der Datenbank zu arbeiten.
...
verbundene Elemente mit löschen müssen wenn kein Verweis mehr darauf
...
nicht in einem Zwischenzustand zurück lassen. Zu diesem Zweck hat man
...
Will ein Prozess zum schreiben zugreifen so muss er zuerst ein Datei
