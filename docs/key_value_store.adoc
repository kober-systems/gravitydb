...
=== Nodes
The `nodes` directory contains the nodes (or vertices). These are our
entry point into the data (when you start searching, it's usually with
a node in most cases. Sometimes you might want to query a property
instead, but for that, you'll either search the entire store (if the
request is unusual) or create an index (and indexes are again nodes).
